"""
Train + eval MUSE-Surv on KIRC (same harness/splits/features as DCMD + baselines).
Full-batch (all patients form one bipartite patient-modality graph).
Loss = Cox + 0.5*unsup-contrastive + 0.5*sup-contrastive (event-time-bin labels).
Reports C-index + IBS + cal-MAD per scenario (P/G/C), 0% and 60%.

Usage: python3 run_muse.py --epochs 100 [--smoke]
"""
...
from muse_surv import MUSESurv  # noqa: E402
...
def time_bins(t, n_bins=4):
    """Quartile bins of survival time -> pseudo-labels for supervised contrastive."""
    edges = np.quantile(t, np.linspace(0, 1, n_bins + 1))
    edges[0] -= 1e-6; edges[-1] += 1e-6
    return np.clip(np.digitize(t, edges) - 1, 0, n_bins - 1)


def train_fold(tr, device, epochs, seed, lr=1e-3, wd=1e-5):
    ...
    model = MUSESurv().to(device)
    opt = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=wd)
    im = _t(tr["img_present"], device); gm = _t(tr["gene_present"], device)
    t = _t(tr["t"], device); e = _t(tr["e"], device)
    ybin = torch.from_numpy(time_bins(tr["t"])).long().to(device)
    model.train()
    for _ in range(epochs):
        opt.zero_grad()
        loss = model(Xi, Xg, im, gm, t, e, ybin)
        loss.backward()
...
def scenario_risk(model, sc_i, sc_g, X_img, X_gene, im_flag, gm_flag, device):
    ...
    model.eval()
    return model.inference(Xi, Xg, a, b).cpu().numpy()
...
def main():
    ...
    if args.smoke:
        return
    lines = ["=" * 64, f"BASELINE: MUSE-Surv (bipartite GNN + contrastive, faithful) — {cohort} 5-fold", "=" * 64]
    ...
    outp = os.path.join(RESULTS_DIR, f"baseline_muse__{cohort}.txt")
